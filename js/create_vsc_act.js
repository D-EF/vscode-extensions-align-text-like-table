 ...
 * @LastEditTime: 2023-02-04 20:35:18
...
function find_Block(){
	...
	for(i=0;i<editor.selections.length;++i){
		if(editor.selections[i].start.line===editor.selections[i].end.line){
			...
			while(j>=0){
				...
				if(!(index_FirstNotSpace(temp_str)===indent)){
					++j;
					break;
				}
				--j;
			...
			str_range=new vscode.Range(new vscode.Position(editor.selections[i].start.line,0),new vscode.Position(editor.selections[i].end.line,editor.selections[i].end.character));
		}
		rtn.push(str_range);
	}
	return rtn;
